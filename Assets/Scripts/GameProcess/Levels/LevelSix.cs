...
public class LevelSix : MonoBehaviour, ILevel
{
    // planet
    ...
    public float waveAsteroidWait = 32;
    // blackhole 
    float spawnBlackholeWait = 20;
    GameObject blackHole;
    Vector3 spawnBlackholePosition;
    float startBlackholePointY = 2000;
    // nebula
    float spawnNebulaWait = 15;
    float startNebulaPointY = 2000;
    Vector3 spawnNebulaPosition;
    GameObject nebula;
    // fuel
    ...
    public IEnumerator NightRoutine()
    {
        Material skyMaterial = controller.skyMaterial;
        Light light = controller.GetComponent<GameController>().light.GetComponent<Light>();
        while (!isLevelStop)
        {
            isFlashlightsGenerate = false;
            controller.StartCoroutine(FlashlightRoutine());
            float exposure = StaticPrefabs.skyIntensity;
            ...
            yield return new WaitForSeconds(nightWait);
        }
    }
    public IEnumerator FlashlightRoutine()
    {
        while (!isLevelStop && !isFlashlightsGenerate)
        {
            if (rocket != null)
            {
                SpawnFlashlight();
    ...
    IEnumerator SpawnNebulas()
    {
        ...
        while (!isLevelStop)
        {
            if (rocket != null)
            {
                ...
                yield return new WaitForSeconds(spawnNebulaWait);
            }
        }
    }
    ...
    {
        float flashlightSize = flashlight.GetComponentsInChildren<MeshFilter>()[0].sharedMesh.bounds.size.x;

        spawnObjectPosition.x = Random.Range(-(gameTunnel) + flashlightSize, gameTunnel - flashlightSize);
        spawnObjectPosition.y = Random.Range(500, 700) + rocket.transform.position.y;
        spawnObjectPosition.z = Random.Range(-(gameTunnel) + flashlightSize, gameTunnel - flashlightSize);
        Instantiate(flashlight, spawnObjectPosition, Quaternion.Euler(-90, 0, 0));
    }
    void SpawnFuel()
    {
